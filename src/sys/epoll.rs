...
impl EpollEvent {
    pub fn new(events: u32, fd: usize) -> Self { Self { events, data: fd as u64 } }
    pub fn fd(&self) -> usize { self.data as usize }
}
impl Default for EpollEvent {
    fn default() -> Self { Self { events: 0, data: 0 } }
}
...
pub fn epoll_add(epfd: usize, fd: usize, events: u32) -> SysResult<()> {
    ...
    let r = unsafe { syscall4(SYS_EPOLL_CTL, epfd, 1 /*ADD*/, fd, &mut ev as *mut _ as usize) };
    if r >= 0 { Ok(()) } else { Err(r) }
}
pub fn epoll_wait(epfd: usize, events: &mut [EpollEvent], timeout_ms: isize) -> SysResult<usize> {
    let r = unsafe { syscall4(SYS_EPOLL_WAIT, epfd, events.as_mut_ptr() as usize, events.len(), timeout_ms as usize) };
    if r >= 0 { Ok(r as usize) } else { Err(r) }
}

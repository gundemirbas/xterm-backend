pub fn sha1(msg: &[u8], out: &mut [u8;20]) {
    ...
    while i < msg.len() {
        let mut bi = 0;
        while bi < 64 && i < msg.len() { block[bi] = msg[i]; bi += 1; i += 1; }
        if bi < 64 {
            block[bi] = 0x80; bi += 1;
            if bi > 56 {
                for k in bi..64 { block[k] = 0; }
                sha1_block(&mut h0,&mut h1,&mut h2,&mut h3,&mut h4,&block);
                for k in 0..56 { block[k] = 0; }
            } else {
                for k in bi..56 { block[k] = 0; }
            }
            for k in 0..8 { block[56+k] = ((ml >> (8*(7-k))) & 0xFF) as u8; }
            sha1_block(&mut h0,&mut h1,&mut h2,&mut h3,&mut h4,&block);
            break;
        } else {
            if i == msg.len() {
...
fn sha1_block(h0: &mut u32, h1: &mut u32, h2: &mut u32, h3: &mut u32, h4: &mut u32, block: &[u8;64]) {
    let mut w = [0u32; 80];
    for i in 0..16 {

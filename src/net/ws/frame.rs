...
pub fn parse_and_unmask_frames<'a>(input: &[u8], out: &'a mut [u8]) -> Result<&'a [u8], &'static str> {
    if input.len() < 2 { return Err("short"); }
    let b0 = input[0];
    let b1 = input[1];
    ...
    if len == 126 {
        if input.len() < idx+2 { return Err("short"); }
        len = ((input[idx] as usize) << 8) | (input[idx+1] as usize);
        idx += 2;
    } else if len == 127 {
        if input.len() < idx+8 { return Err("short"); }
        let mut l: u64 = 0;
        for i in 0..8 { l = (l << 8) | (input[idx+i] as u64); }
        idx += 8;
        len = l as usize;
    }
    if !masked { return Err("client not masked"); }
    if input.len() < idx+4+len { return Err("short"); }
    ...
    let opcode = b0 & 0x0F;
    if opcode == 0x8 { return Err("close"); }
    Ok(&out[..len])
}
